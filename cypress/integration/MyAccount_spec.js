describe('The MyAccount page', function() {
  beforeEach(function() {
  ...
  it('should show MyAccount page after disclosing attribute completes with valid proof', function() {
    cy.route('GET', '/api/disclosure-status*', 'fixture:api/disclosure-status/completed-valid').as('disclosure-status');
    cy.route('GET', '/api/get-session', 'fixture:api/get-session/email').as('get-session');
    cy.wait('@disclosure-status');
    cy.get('#disclosure-proof-completed');
    cy.wait('@get-session');
    cy.get('#my-account-page');
  });
